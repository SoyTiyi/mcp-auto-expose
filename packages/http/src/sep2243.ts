...
export function validateSep2243(
  ...
  if (
    ...
    return { ok: false, reason: "malformed-body", detail: "JSON-RPC body must be an object with a string 'method' field" };
  }
  ...
  if (mcpMethod === undefined) {
    return { ok: false, reason: "missing-header", detail: "Mcp-Method header is required for POST requests" };
  }

  // Rule 3: mcp-method must match body.method (case-sensitive)
  if (mcpMethod !== bodyMethod) {
    return { ok: false, reason: "method-mismatch", detail: `Mcp-Method header value '${mcpMethod}' does not match body method '${bodyMethod}'` };
  }
  ...
  if (bodyMethod === "tools/call") {
    ...
    if (mcpName === undefined) {
      return { ok: false, reason: "missing-header", detail: "Mcp-Name header is required for tools/call" };
    }
    ...
    if (mcpName !== bodyName) {
      return { ok: false, reason: "name-mismatch", detail: `Mcp-Name header value '${mcpName}' does not match body params.name '${bodyName ?? ""}'` };
    }
